...
export function hydrate(Component: Function, container: Element | null): void {
  if (!container) {
    console.warn("[Vibeframe] Hydration target #__vibeframe not found");
    return;
  }

  const dataEl = document.getElementById("__VIBEFRAME_DATA__");
  ...
  console.log("[Vibeframe] Hydration complete");
}
